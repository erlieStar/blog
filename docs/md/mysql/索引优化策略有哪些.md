...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/fb1df947bfa186ae9e91829b8bc2c259.png)
## 索引的种类
...
**当字符串和数字进行比较时，MySQL 的规则是将字符串转成数字**，而不是将数字转成字符串
...
select * from article where id = '100'
```
**答案是会用到索引**，因为如下两个语句是等价的

```sql
select * from article where id = '100'

select * from article where id = CAST('100' AS signed int)
```

在这个过程中，函数/转换操作是作用在“常量（查询条件）”上，而不是作用在“索引列 id”上。id 列本身没有发生任何函数改变，B+ 树依然可以通过有序的数字100快速进行二分查找定位。因此，这种情况可以正常走索引
### 前导模糊查询不会使用索引
