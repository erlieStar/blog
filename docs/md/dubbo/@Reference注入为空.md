...
**线上dubbo的版本为2.5.10，注意包名是（com.alibaba）**
...
```java
// com.alibaba.dubbo.config.ReferenceConfig#createProxy 中的部分代码
...
用的dubbo的版本为2.5.10，注意包名是（com.alibaba）
...
public PropertyValues postProcessPropertyValues(
    ...
    // findReferenceMetadata 方法会通过 Java 反射机制，扫描这个类（以及它的父类）中所有加了 @Reference 注解的成员变量（Fields）和方法（Methods），扫描出来的结果会被封装成一个 InjectionMetadata 对象
...
当check属性设置为false时，第一次调用成功执行完ReferenceBean#getObject方法，ref能正常赋值为代理对象，因为不会在校验过程中抛出IllegalStateException异常
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/bd1263c86119412bb34bc3753ae6bd1a.png)

## 解决方案
1. @Reference注解的check属性设置为false（默认为true），因为当你的check属性为true并且没有服务提供者时，不会起任何作用，对象为null，后续当有服务提供者可用时，这个对象始终为null。当check为false时，会注入一个代理对象，当有服务提供者时，这个代理对象会刷新，就能正常发起调用
2. 选择能正常执行的版本