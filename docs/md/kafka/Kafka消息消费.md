...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/2d7c65c15d3229e7c150fdf4d9783ca6.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/74205b7047f580e9842e41527f448e9d.png)
...
首先对 groupId 进行 hash，接着对 __consumer_offsets 的分区数量取模，默认是50，找到分区对应的 leader 所在的 broker，这个 broker 就是 consumer group 的 Coordinator，接着每个 consumer 就会维护一个 socket 跟这个 broker 进行通信
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/52ae565c355db94eedfd2990dfd4ffea.png)


![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/556e4fec9d593035b225daf039e70268.png)

**RoundRobinAssignor**：针对集群中所有 topic 而言，把所有的 partition 和 所有的 consumer 都列出来，然后按照 hashcode 进行排序，最后通过轮询算法将 partition 分配给各个消费者

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/74f15ce7ffd3354379bf5106e3c05354.png)
...
可以配置的参数可以参考如下文档
https://kafka.apache.org/0110/configuration/consumer-configs/
...
## 位移提交
### 执行流程
每个 consumer 对于被分配到的每个分区，都会在内存中维护一个逻辑指针，叫做position，position记录了 consumer 下一次将要拉取的消息的 offset

执行流程如下
1. 初始状态：Consumer 刚启动，position 没有值。
2. 决定起始位置：它会根据配置的 **auto.offset.reset** 策略来确定一个初始值：
3. 发送拉取请求：Consumer 将 position 包含在 FetchRequest 中发给 Broker。
4. 获取消息：Broker 从指定的 Offset 开始返回消息。
5. 更新 position：Consumer 收到消息后，会根据返回的消息条数，自动将 position 向后累加。
6. 重复步骤 3-5：下一次 poll() 就继续从新的 position 拉取。

注意：position 的更新与 Offset 提交无关。它只是纯粹的内存游标，用于决定下一次拉取位置。即使从不提交 Offset，position 也会随着消息被消费而不断前移。

当消费者增加、减少或崩溃时，会触发分区重分配，这时 Offset 不会丢失，原因如下
1. 分配前：每个 Consumer 会将其负责的各个分区的 position 提交到 Broker（存于 __consumer_offsets 主题）。

2. 重新分配后：新获得该分区的 Consumer，会从 Broker 读取之前提交的 committed offset，据此初始化自己的 position
### 细节分析
consumer会将消费的位移信息提交到 __consumer_offsets 这个 topic，默认分区是50个。

消息格式如下图所示
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/9a32fd6d1a12f9bc59afbafcae05c0cd.png)
...
| 可选值 | 描述 |
|--|--|
| earliest | 从分区开头 (Offset 0) 开始 |
| latest | 从分区末尾（只拉取新产生的消息）开始 |
| none | 如果没有已提交的 Offset，则抛出异常 |

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b8c4453a6231622cab726ebef2b01804.png)
### 自动提交
