...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/ddec703917ed46fd8011c754e84a5d73.png)
## 架构设计
### 复制集架构
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/8f47f215c0324f79a7bbe8e4b4bcf4fa.png)

在谈具体的读写前，必须先理解**复制集（Replica Set）**。它是 MongoDB 保证数据安全和自动容灾的基础。
...
### 分片集群架构
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/eb4c13f07a104cf0a9d4958daa69c57b.png)
...
1. 内存更新：数据首先写入 WiredTiger 的内存缓冲区（Cache），并在内存中修改 B +树索引。
...
3. 写 Oplog：同时，该操作会被记录到 local.oplog.rs 集合中，用于后续的节点间复制（也是先写入内存再落盘）

4. 返回客户端：根据客户端配置的 Write Concern（写安全级别），决定何时向客户端返回成功信号。

5. Data Checkpoint（异步刷盘）：
- 内存中的脏数据并不会立刻写入真正的数据文件。
- WiredTiger 默认每 60 秒（或脏数据达到一定比例）做一次 Checkpoint，将内存中的修改真正同步到磁盘的数据文件中。

在 MongoDB 中，local.oplog.rs 虽然名字叫“操作日志”，但它在底层的底层实现上，本质上就是一个普通的 MongoDB 集合（Collection）。因此，它的读写逻辑与普通的数据集合完全是一模一样的
...
1. 检查 Cache：WiredTiger 引擎首先看请求的数据页（Page）是否在内存 Cache 中。

2. 磁盘加载：如果命中，直接从内存返回；如果未命中，产生 Page Fault（缺页中断），从磁盘数据文件中将数据页加载到 Cache 中。

3. 索引加速：如果有索引，会通过 B-Tree 索引快速定位数据页；如果是全表扫描（COLLSCAN），则需要加载大量数据页。
